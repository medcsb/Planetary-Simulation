...
inline std::string u8path_to_string(const std::filesystem::path& p) {
    auto u8 = p.u8string();
...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			for (auto comp : path) {
				std::string section = u8path_to_string(comp);
			...
			if (!anyOtherHC && clicked) {
				auto temp = u8path_to_string(path);
				strcpy(pathBuffer, reinterpret_cast<const char*>(temp.c_str()));
				*state |= 0b001;
	...
	FileDialog::FileData::FileData(const std::filesystem::path& path) {
		...
		struct stat attr;
		stat(u8path_to_string(path).c_str(), &attr);
	...
	FileDialog::FileDialog() {
		...
		for (const auto& entry : std::filesystem::directory_iterator("/", ec)) {
			if (std::filesystem::is_directory(entry, ec))
				thisPC->Children.push_back(new FileTreeNode(u8path_to_string(entry.path())));
		}
		m_treeCache.push_back(thisPC);
#endif
	}
	...
	{
		auto itr = std::find(m_favorites.begin(), m_favorites.end(), u8path_to_string(m_currentDirectory));
	...
	void FileDialog::m_select(const std::filesystem::path& path, bool isCtrlDown)
	{
		...
		if (m_selections.size() == 1) {
			std::string filename = u8path_to_string(m_selections[0].filename());
			if (filename.size() == 0)
				filename = u8path_to_string(m_selections[0].filename());
			...
			for (const auto& sel : m_selections) {
				std::string filename = u8path_to_string(sel.filename());
				if (filename.size() == 0)
					filename = u8path_to_string(sel.filename());
	...
	{
#ifdef _WIN32
		if (m_icons.count(u8path_to_string(path)) > 0)
			return m_icons[u8path_to_string(path)];

		std::string pathU8 = u8path_to_string(path);
	...
	void FileDialog::m_loadPreview()
	{
		for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
			...
			if (data.Path.has_extension()) {
				std::string ext = u8path_to_string(data.Path.extension());
				if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga") {
					int width, height, nrChannels;
					unsigned char* image = stbi_load(u8path_to_string(data.Path).c_str(), &width, &height, &nrChannels, STBI_rgb_alpha);
	...
	void FileDialog::m_setDirectory(const std::filesystem::path& p, bool addHistory)
	{
		...
		std::string pathStr(reinterpret_cast<const char*>(u8.c_str()), u8.size());

		if (pathStr.size() == 2 && pathStr[1] == ':')
			m_currentDirectory = std::filesystem::u8path(pathStr + "\\");
#endif
		...
		if (u8path_to_string(p) == "Quick Access") {
		...
		else if (u8path_to_string(p) == "This PC") {
			for (auto& node : m_treeCache) {
				if (node->Path == p)
					for (auto& c : node->Children)
						m_content.push_back(FileData(c->Path));
			...
			if (std::filesystem::exists(m_currentDirectory, ec))
				for (const auto& entry : std::filesystem::directory_iterator(m_currentDirectory, ec)) {
					...
					if (m_searchBuffer[0]) {
						std::string filename = u8path_to_string(info.Path);
					...
					if (!info.IsDirectory && m_type != IFD_DIALOG_DIRECTORY) {
						if (m_filterSelection < m_filterExtensions.size()) {
							...
							if (exts.size() > 0) {
								std::string extension = u8path_to_string(info.Path.extension());
	...
	void FileDialog::m_sortContent(unsigned int column, unsigned int sortDirection)
	{
		...
		if (m_content.size() > 0) {
			...
			auto compareFn = [column, sortDirection](const FileData& left, const FileData& right) -> bool {
				// name
				if (column == 0) {
					std::string lName = u8path_to_string(left.Path);
					std::string rName = u8path_to_string(right.Path);
	...
	void FileDialog::m_renderTree(FileTreeNode* node)
	{
		...
		std::string displayName = u8path_to_string(node->Path.stem());
		if (displayName.size() == 0)
			displayName = u8path_to_string(node->Path); // drive
			...
			if (!node->Read) {
				...
				if (std::filesystem::exists(node->Path, ec))
					for (const auto& entry : std::filesystem::directory_iterator(node->Path, ec)) {
						if (std::filesystem::is_directory(entry, ec))
							node->Children.push_back(new FileTreeNode(u8path_to_string(entry.path())));
					}
				node->Read = true;
			}
	...
	void FileDialog::m_renderContent()
	{
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					std::string filename = u8path_to_string(entry.Path.filename());
					if (filename.size() == 0)
						filename = u8path_to_string(entry.Path); // drive
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				ImTextureID texId;
				if (entry.HasIconPreview)
					texId = (ImTextureID)(uintptr_t)entry.IconPreview;
				else
					texId = (ImTextureID)(uintptr_t)m_getIcon(entry.Path);
	...
	void FileDialog::m_renderPopups()
	{
		...
		if (ImGui::BeginPopupModal("Are you sure?##delete")) {
			if (m_selectedFileItem >= static_cast<int>(m_content.size()) || m_content.size() == 0)
				...
				ImGui::TextWrapped("Are you sure you want to delete %s?", u8path_to_string(data.Path.filename()).c_str());
				if (ImGui::Button("Yes")) {
					std::error_code ec;
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (FavoriteButton("##dirfav", std::count(m_favorites.begin(), m_favorites.end(), u8path_to_string(m_currentDirectory)))) {
			if (std::count(m_favorites.begin(), m_favorites.end(), u8path_to_string(m_currentDirectory)))
				RemoveFavorite(u8path_to_string(m_currentDirectory));
			else 
				AddFavorite(u8path_to_string(m_currentDirectory));
		}
		ImGui::SameLine();
		ImGui::PopStyleColor();
